  ...
  async fetch() {
    ...
    try {
      ...
      const responseC = await axios.get(
        `http://localhost:8000/events/${event_id}/criteria`,
        {
          headers: {
  ...
  methods: {
    async changeSelectedParticipants(ParticipantId) {
      ...
      try {
        ...
        //запрос на оценку
        const response = await axios.get(
          `http://localhost:8000/get_score?nomination_id=${nomination_id}&expert_id=${this.user.id}&application_id=${ParticipantId}`,
          {
            headers: {
              Authorization: "Bearer " + this.$cookies.get("token"),
            },
    ...
    async saveEvaluation() {
      try {
        ...
        const response = await axios.put(
          `http://localhost:8000/update_scores?eventId=${this.blank.eventId}`,
          {
            ...this.blank,
